...
def plot_results(k,data,xs,rhos):
    ...
    pylab.xlim(-2.0, 2.0)
    pylab.title('Path-integral Monte Carlo algorithm for the harmonic oscillator for slice {0}'.format(k), fontsize = 12)
    pylab.xlabel('$x$', fontsize = 30)
    pylab.ylabel('$Probability$', fontsize = 20)
    pylab.legend(loc='upper left',framealpha=0.5)
    pylab.savefig('b2_{0}.png'.format(k).format(beta)) 
    
if __name__=='__main__':
    (xs,rhos)=read_file('data_harm_matrixsquaring_beta_4.0.dat')
